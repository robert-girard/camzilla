...
- 2026-07-18 completion audit: Backup restore now checks the optimistic version before warming a model and serializes any runtime switch with the database commit. Regression coverage proves a stale restore does not load a candidate, candidate failure leaves runtime and persistence unchanged, commit failure rolls the runtime back, and startup persists a redacted bootstrap fallback when the saved model cannot load.
...
- [x] Extend model/backend capabilities with a versioned class catalog containing stable semantic IDs, model-native class IDs, display labels, and optional descriptions; do not persist model-specific numeric indices as the cross-model identity.
