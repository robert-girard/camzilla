...
- **Detection categories:** Phase 3b exposes the active artifact's versioned class catalog for per-camera and per-alert-rule multi-selection. Catalog entries carry a stable semantic ID, model-native ID, display label, and optional description. These are object-detection classes with bounding boxes, not image-classification categories. Publication, metrics, media annotation/event capture, and alert evaluation filter by stable semantic ID, while adapters own model-native index mapping. Model-switch preflight intersects saved IDs with the target catalog and rejects missing selections before loading the candidate. Events and backups retain the catalog revision; historical display resolves known stable IDs and otherwise shows the ID plus unavailable revision without guessing a label or substitute.
